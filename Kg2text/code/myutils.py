...
def get_my_args():
    ...
    parser.add_argument("--seed", type=int, default=0, help="specify a random seed")
    parser.add_argument("--data", type=str, default="test", help="specify webnlg/wikidata/wikibionlg")
    parser.add_argument('--split', type=str, default="eval", help="option: train/eval/test")
    parser.add_argument("--option", type=str, default="finetune", help="option: train/finetune/eval/test")
    ...
    parser.add_argument("--experiment_comment", type=str, default="", help="experiment comment")
    
    parser.add_argument("--encoder_type", type=str, default="", help="")
    parser.add_argument('--decoder_type', type=str, default="", help="")
    parser.add_argument("--tokenizer_src", type=str, default="", help="")
    parser.add_argument('--tokenizer_tgt', type=str, default="", help="")
    parser.add_argument("--vocab_file_src", type=str, default="", help="")
    parser.add_argument('--vocab_file_tgt', type=str, default="", help="")
    parser.add_argument("--pretrained_encoder_file", type=str, default="", help="")
    parser.add_argument('--pretrained_decoder_file', type=str, default="", help="")
    parser.add_argument("--save_encoder_file", type=str, default="", help="")
    parser.add_argument("--save_decoder_file", type=str, default="", help="")
    parser.add_argument("--save_every_n_epochs", type=int, default=1, help="")
    
    parser.add_argument("--src_length", type=int, default=256, help="")
    parser.add_argument("--tgt_length", type=int, default=72, help="")
    parser.add_argument("--prepend_src_lang_tag", type=bool, default=False, help="")
    parser.add_argument("--prepend_tgt_lang_tag", type=bool, default=False, help="")
    parser.add_argument("--src_wtags", type=bool, default=True, help="")
    parser.add_argument("--tgt_wtags", type=bool, default=True, help="")

    parser.add_argument("--use_copy_gate", type=bool, default=True, help="")
...
def get_kg2text_abs_path(label, cfg, path, project="Kg2text", project_path = None):
    ...
    if label == "dict":
        ...
        # create checkpoint/path/ folder for storing cp files and results
...
def abspath_model_cfg(cfg, model_cfg, data_cfg):
    ...
    abs_knowledge_file = get_kg2text_abs_path("knowledge_file", cfg.model.kgpt, model_cfg.kgpt.knowledge_file)
    model_cfg.kgpt.knowledge_file = abs_knowledge_file
    data_cfg.knowledge_file = abs_knowledge_file
...
def check_cfg(cfg):
    ...
    if "kgpt" in task_cfg.encoder_type:
        assert data_cfg.tokenizer_src in ["kgpt_gpt2", "fairseq_gpt2"]
        assert "gpt" in data_cfg.vocab_file_src
    elif "mbart" in task_cfg.encoder_type:
        assert data_cfg.tokenizer_src in ["mbart", "mbart50", "sentencepiece"]
        assert "mbart" in data_cfg.vocab_file_src
    if "wtags" in task_cfg.encoder_type:
        assert "wtags" in data_cfg.vocab_file_src

    if "kgpt" in task_cfg.decoder_type:
        assert data_cfg.tokenizer_tgt in ["kgpt_gpt2", "fairseq_gpt2"]
        assert "gpt" in data_cfg.vocab_file_tgt
    elif "mbart" in task_cfg.decoder_type:
        assert data_cfg.tokenizer_tgt in ["mbart", "mbart50", "sentencepiece"]
        assert "mbart" in data_cfg.vocab_file_tgt
    if "wtags" in task_cfg.decoder_type:
        assert "wtags" in data_cfg.vocab_file_tgt

    if data_cfg.tokenizer_src != data_cfg.tokenizer_tgt:
        model_cfg.share_all_embeddings = False

    if "mbart" in model_cfg.decoder_type:
        decoder_specify = "mbart50"
    elif "kgpt" in model_cfg.decoder_type:
        decoder_specify = "kgpt"
    model_cfg.encoder_embed_dim = getattr(getattr(model_cfg, model_cfg.encoder_type, "kgpt"), "decoder_embed_dim", 0)
    model_cfg.decoder_embed_dim = getattr(getattr(model_cfg, decoder_specify, "kgpt"), "decoder_embed_dim", 0)
    """
    # token_cfg == data_cfg
    ...
    if model_cfg.pretrained_encoder_file:
        assert model_cfg.encoder_type in model_cfg.pretrained_encoder_file
    if model_cfg.pretrained_decoder_file:
        assert model_cfg.decoder_type in model_cfg.pretrained_decoder_file
    ...
    def backup():
        if model_cfg.encoder_type == "kgpt":
            ...
            if model_cfg.decoder_type == "kgpt":
                ...
                if model_cfg.pretrained_encoder_file:
                    assert "kgpt" in model_cfg.pretrained_encoder_file
                if model_cfg.pretrained_decoder_file:
                    assert "kgpt" in model_cfg.pretrained_decoder_file
                """
...
def set_cfg(cfg_new, args, now):
    cfg_load = load_my_cfg(args.config)
    cfg_load.task.split, cfg_load.task.option, cfg_load.task.dataset = args.split, args.option, args.data
    ...
    task_cfg.encoder_type = args.encoder_type
    task_cfg.decoder_type = args.decoder_type
    # data_cfg = getattr(task_cfg, task_cfg.dataset)
    data_cfg.split = task_cfg.split
    data_cfg.option = task_cfg.option
    data_cfg.prepend_src_lang_tag = args.prepend_src_lang_tag
    data_cfg.prepend_tgt_lang_tag = args.prepend_tgt_lang_tag
    data_cfg.src_wtags = args.src_wtags
    data_cfg.tgt_wtags = args.tgt_wtags
    data_cfg.encoder_type = args.encoder_type
    data_cfg.decoder_type = args.decoder_type
    data_cfg.tokenizer_src = args.tokenizer_src
    data_cfg.tokenizer_tgt = args.tokenizer_tgt
    data_cfg.vocab_file_src = args.vocab_file_src
    data_cfg.vocab_file_tgt = args.vocab_file_tgt
    
    
    abspath_data_cfg(data_cfg)

    # for model
    model_cfg.encoder_type, model_cfg.decoder_type = args.encoder_type, args.decoder_type
    model_cfg.pretrained_encoder_file, model_cfg.pretrained_decoder_file = args.pretrained_encoder_file, args.pretrained_decoder_file
    model_cfg.save_encoder_file, model_cfg.save_decoder_file = args.save_encoder_file, args.save_decoder_file
    model_cfg.use_copy_gate = args.use_copy_gate
    # for common
    cfg.common.save_every_n_epochs = args.save_every_n_epochs
    cfg.common.learning_rate = args.learning_rate
    check_cfg(cfg)
    # = args.load_from
    abspath_model_cfg(cfg, model_cfg, data_cfg)
    set_output_dir(cfg, args, now)
    return cfg
    # common config

def set_output_dir(cfg: Kg2textConfig, args, now):
    """
    ...
    folder = "{}_{}_{}_{}_{}_{}".format(encoder_type, decoder_type, task_cfg.option, task_cfg.dataset, args.experiment_comment, now)
...
def paraFromKgpt3(model_new,model_trained):
    ...
    for key_new in list_new:
        # suffix = ".".join(key_new.split(".")[1:])
        suffix = key_new
        if "output_projection" in suffix:
            suffix = removeprefix(suffix, "output_projection.")
        matching = [s for s in list_trained if suffix in s]

...
class KnowledgeEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, entity_ids, triple_ids, position_ids):
        if input_ids is not None:
            ...
            input_shape = inputs_embeds.size()[:-1] # TODO: What it is
...
def get_attn_key_pad_mask(seq_k, seq_q, pad_idx): # seq_k and seq_q are both tgt_seq
    ''' For masking out the padding part of key sequence. '''

    # Expand to fit the shape of key query attention matrix.
    len_q = seq_q.size(1)
    padding_mask = seq_k.eq(pad_idx) # seq_k: [bsz, 1], [bsz,2],..., [bsz, max_token_seq_len]
    padding_mask = padding_mask.unsqueeze(1).expand(-1, len_q, -1)  # b x lq x lk

    return padding_mask # [bsz,1,1], [bsz,2,2],....
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_input, enc_output, non_pad_mask=None, slf_attn_mask=None, dec_enc_attn_mask=None):
        ...
        dec_output *= non_pad_mask # [2,1,768] *= [2,1,1]
...
class GatedTransformerDecoder(nn.Module):
    ...
    def greedy_decode(self, input_ids, entity_ids, triple_ids, position_ids, banwords=[], max_token_seq_len=30):
        ...
        batch_size = input_ids.shape[0]
        tgt_seq = torch.LongTensor(batch_size, 1).fill_(self.sos_idx).to(input_ids.device) # [bsz,1], [[sos_idx],..., [sos_idx]]

        for step in range(max_token_seq_len):
            non_pad_mask = get_non_pad_mask(tgt_seq, self.pad_idx) # [bsz,1]  [[True],...,[True]]

            slf_attn_mask_subseq = get_subsequent_mask(tgt_seq)
            slf_attn_mask_keypad = get_attn_key_pad_mask(seq_k=tgt_seq, seq_q=tgt_seq, pad_idx=self.pad_idx)
            slf_attn_mask = (slf_attn_mask_keypad + slf_attn_mask_subseq).gt(0) # entry >0 -> True else False
            dec_enc_attn_mask = get_attn_key_pad_mask(seq_k=input_ids, seq_q=tgt_seq, pad_idx=self.pad_idx) # [bsz, len_q, len_k], [2,1,760]
            ...
            scores = torch.bmm(dec_output, enc_inp.transpose(2, 1)) # TODO? why enc_inp (== enc_out)
            scores = scores.masked_fill(dec_enc_attn_mask, -np.inf)
...
class KgptEncoder(nn.Module):

    def __init__(self, config, n_head, n_layers):
        super(KgptEncoder, self).__init__()
        
        d_k = config.hidden_size // n_head
        d_v = config.hidden_size // n_head
        d_inner = config.hidden_size * 4
        self.pad_idx_enc = config.pad_token_id
        self.sos_idx_enc = config.sos_token_id
        self.eos_idx_enc = config.eos_token_id
        ...
        # encoder.enc_layer_stack[0]

    def forward(self, input_ids, entity_ids, triple_ids, position_ids):
        enc_emb = self.embedding(input_ids, entity_ids, triple_ids, position_ids)
        enc_inp = enc_emb.copy()

        # -- Encode source
        non_pad_mask = get_non_pad_mask(input_ids, self.pad_idx_enc)
        slf_attn_mask = get_attn_key_pad_mask(seq_k=input_ids, seq_q=input_ids, pad_idx=self.pad_idx_enc)
        for layer in self.enc_layer_stack:
            enc_inp, _ = layer(enc_inp, non_pad_mask, slf_attn_mask)
        enc_output = enc_inp

        return {
            "encoder_out": [enc_output],  # T x B x C
            "encoder_padding_mask": [non_pad_mask],  # B x T TODO: check pad->False or pad->True
            "encoder_embedding": [enc_emb],  # B x T x C
            "encoder_states": None,  # List[T x B x C] # TODO: [] or None?
            "src_tokens": [],
            "src_lengths": [],
        }
...
class KgptDecoder(nn.Module):
    ...
    def __init__(self, config, n_head, n_layers):
        ...
        d_v = config.hidden_size // n_head
        d_inner = config.hidden_size * 4
        self.pad_idx_enc = config.pad_token_id_enc
        self.sos_idx_enc = config.sos_token_id_enc
        self.eos_idx_enc = config.eos_token_id_enc
        self.pad_idx_dec = config.pad_token_id_dec
        self.sos_idx_dec = config.sos_token_id_dec
        self.eos_idx_dec = config.eos_token_id_dec
    ...
    def forward(self, input_ids, tgt_seq, encoder_out, positionwise_copy_prob=False):
        # TODO wrap xxx_ids
        enc_output, enc_emb = encoder_out["encoder_out"][0], encoder_out["encoder_embedding"][0]
        # -- Prepare masks
        non_pad_mask = get_non_pad_mask(tgt_seq, self.pad_idx_dec)

        slf_attn_mask_subseq = get_subsequent_mask(tgt_seq)
        slf_attn_mask_keypad = get_attn_key_pad_mask(seq_k=tgt_seq, seq_q=tgt_seq, pad_idx=self.pad_idx_dec)
        slf_attn_mask = (slf_attn_mask_keypad + slf_attn_mask_subseq).gt(0)
        dec_enc_attn_mask = get_attn_key_pad_mask(seq_k=enc_emb, seq_q=tgt_seq, pad_idx=self.pad_idx_enc)
        ...
        scores = torch.bmm(dec_output, enc_output.transpose(2, 1)) # TODO: enc_inp->enc_output
        scores = scores.masked_fill(dec_enc_attn_mask, -np.inf)
    ...
    def greedy_decode(self, input_ids, encoder_out, banwords=[], max_token_seq_len=30):
        enc_output, enc_emb = encoder_out["encoder_out"][0], encoder_out["encoder_embedding"][0]
    ...
    def beam_search(self, inputs, tokenizer, n_bm, max_token_seq_len=30, banwords_dec=[]):
        with torch.no_grad():
            # -- Repeat data for beam search
            n_inst, len_s = inputs[0].size()
            ...
            for len_dec_seq in range(1, max_token_seq_len + 1):
                active_inst_idx_list = beam_decode_step(self, inst_dec_beams, len_dec_seq, active_inst_idx_list,
                                                        inputs, inst_idx_to_position_map, n_bm, banwords_dec)
...
class KgptGatedTransformer(nn.Module):
    # TODO: 1. Train from scratch on [EN] / [Ru] 2 Load pretrained and finetune on [Ru],[En]
    def __init__(self, config, n_head, n_layers):
        super(KgptGatedTransformer, self).__init__()

        self.kgptEncoder = KgptEncoder(config, n_head, n_layers)
        self.kgptDecoder = KgptDecoder(config, n_head, n_layers)

    def forward(self, input_ids, entity_ids, triple_ids, position_ids, tgt_seq, positionwise_copy_prob=False):

        encoder_out = self.kgptEncoder(input_ids, entity_ids, triple_ids, position_ids)
        decoder_out = self.kgptDecoder(tgt_seq, encoder_out, positionwise_copy_prob=False)
        # TODO: complete the return part according to fairseq
        return decoder_out
